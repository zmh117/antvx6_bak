...
RETIRED_BPMN_FIELDS = {
    "description", "actor", "businessRule", "business_rule",
    "inputSummary", "input_summary", "outputSummary", "output_summary",
    "semanticProfileKey", "semantic_profile_key",
    "semanticProfileVersion", "semantic_profile_version",
    "semanticPayloadJson", "semantic_payload_json",
    "processContainerJson", "process_container_json",
    "containerNodeKey", "container_node_key",
    "bpmnCallActivityRef", "bpmn_call_activity_ref",
    "conditionText", "condition_text",
    "dataContractJson", "data_contract_json",
    "bpmnMessageName", "bpmn_message_name",
    "bpmnConditionExpression", "bpmn_condition_expression",
}


def _reject_retired_bpmn_fields(payload: dict[str, Any], target_key: str) -> None:
    retired = sorted(RETIRED_BPMN_FIELDS.intersection(payload))
    if retired:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"retired BPMN fields for {target_key}: {', '.join(retired)}",
        )
...
def _fetch_business_flow_editor_state(
    ...
    cur.execute(
        ...
        SELECT e.id, e.lane_instance_id, e.edge_key, e.source_type,
               ...
               e.target_port, e.edge_type, e.label,
...
def _apply_business_flow_op(cur, business_flow_id: UUID, op) -> None:
    ...
    if op.target_type in {"NODE", "EDGE"}:
        _reject_retired_bpmn_fields(patch, target_key)
    ...
    if op.op_type == "ADD_NODE":
        ...
        cur.execute(
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, TRUE, %s, %s, %s, %s)
    ...
    if op.op_type == "ADD_EDGE":
        ...
        cur.execute(
            ...
            INSERT INTO business_flow_edge (
                ...
                edge_type, label, is_overridden,
                bpmn_flow_type, bpmn_sequence_flow_kind,
                style_json, properties_json,
                bpmn_semantic_json
            )
            VALUES (%s, %s, %s, 'NODE', %s, %s, 'NODE', %s, %s, %s, %s, TRUE, %s, %s, %s, %s, %s)
...
def _build_business_flow_materialize_ops(
    ...
    for key, node in incoming_nodes.items():
        _reject_retired_bpmn_fields(node, key)
    ...
    for key, edge in incoming_edges.items():
        _reject_retired_bpmn_fields(edge, key)
    ...
    with db_transaction() as conn:
        with conn.cursor() as cur:
            ...
            for component_node in component_nodes:
                ...
                node_position_x = LANE_PADDING_LEFT + (
                    float(component_node["position_x"]) - min_x
                )
                node_position_y = LANE_HEADER_HEIGHT + (
                    float(component_node["position_y"]) - min_y
                )
                cur.execute(
                    """
                    INSERT INTO business_flow_node (
                        business_flow_id, lane_instance_id, node_key,
                        origin_component_node_key, node_type, title,
                        bpmn_element_type, bpmn_event_kind, bpmn_event_definition,
                        bpmn_task_type, bpmn_gateway_type, bpmn_subprocess_kind,
            ...
            for component_edge in component_edges:
                ...
                cur.execute(
                    ...
                    INSERT INTO business_flow_edge (
                        ...
                        target_port, edge_type, label,
                        bpmn_flow_type, bpmn_sequence_flow_kind,
                        origin_component_edge_key, style_json, properties_json,
                        bpmn_semantic_json
                    )
                    VALUES (%s, %s, %s, 'NODE', %s, %s, 'NODE', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """,
                    (
                        business_flow_id,
                        lane_id,
            ...
            for edge in edges:
                ...
                cur.execute(
                    ...
                    INSERT INTO business_flow_edge (
                        ...
                        bpmn_flow_type, bpmn_sequence_flow_kind, label,
                        origin_component_edge_key,
